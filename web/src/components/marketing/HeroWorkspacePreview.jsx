...
export default function HeroWorkspacePreview({ className = "" }) {
  ...
  return (
    <motion.div
      ...
      className={`rounded-3xl border border-neutral-200/90 bg-white/95 p-3 sm:p-4 lg:p-5 shadow-[0_2px_0_rgba(15,23,42,0.02),0_30px_60px_-24px_rgba(15,23,42,0.2)] ${className}`}
    >
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[minmax(260px,0.9fr)_minmax(0,1.32fr)] xl:grid-cols-[minmax(280px,0.88fr)_minmax(0,1.38fr)] gap-3 sm:gap-4 lg:gap-5 items-stretch min-h-0">
        <aside className="rounded-2xl border border-neutral-200/90 bg-[#fafbfc] overflow-hidden min-h-[420px] md:min-h-[560px] lg:min-h-[620px] xl:min-h-[680px] h-full flex flex-col">
        ...
        <section className="rounded-2xl border border-neutral-200/90 bg-gradient-to-b from-white to-neutral-50/40 overflow-hidden min-h-[420px] md:min-h-[560px] lg:min-h-[620px] xl:min-h-[680px] h-full flex flex-col min-w-0 shadow-[inset_0_1px_0_rgba(255,255,255,0.9)]">
          <header className="px-5 py-4 sm:px-6 sm:py-5 lg:px-7 lg:py-6 border-b border-neutral-200/90 bg-white/80 shrink-0 space-y-4">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between lg:gap-6">
              <div className="min-w-0 flex-1 space-y-2.5">
                <h3 className="text-xl sm:text-2xl lg:text-[1.625rem] xl:text-[1.75rem] font-semibold tracking-tight text-neutral-900 leading-[1.2] text-balance">
                  {activeRole.title}
                </h3>
                <p className="text-sm sm:text-[15px] text-neutral-500 leading-snug">
                  {activeRole.company} · {activeRole.location} · Full-time
                </p>
                <p className="text-base sm:text-lg font-semibold tabular-nums text-emerald-800 tracking-tight">
                  {activeRole.salary}
                </p>
              </div>
              <div className="flex flex-wrap items-center gap-2.5 shrink-0 lg:pt-1">
                <button
                  type="button"
          ...
          <div className="px-5 py-5 sm:px-6 sm:py-6 lg:px-7 lg:py-7 border-b border-neutral-200/80 flex-1 min-h-0 overflow-y-auto bg-white/50">
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-neutral-400 mb-3">
              About the role
            </p>
            <p className="text-sm sm:text-[15px] lg:text-base text-neutral-600 leading-[1.65] max-w-prose text-pretty">
              {activeRole.about}
            </p>
          </div>

          <div className="mx-4 my-3 sm:mx-5 sm:my-4 lg:mx-6 lg:my-5 shrink-0 rounded-2xl border border-amber-200/70 bg-gradient-to-br from-amber-50/90 to-amber-100/40 px-4 py-4 sm:px-5 sm:py-5 shadow-sm">
          ...
          <footer className="px-5 py-3.5 sm:px-6 sm:py-4 lg:px-7 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 text-sm text-neutral-600 bg-white/70 border-t border-neutral-200/80 mt-auto shrink-0">
            <div className="flex items-start gap-2.5 min-w-0">
              <MapPin size={16} className="shrink-0 text-neutral-400 mt-0.5" aria-hidden />
              <span className="leading-snug text-pretty sm:max-w-[min(100%,22rem)] lg:max-w-none">
